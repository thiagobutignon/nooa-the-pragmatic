...
export function InitScreen({ onBack }: InitScreenProps) {
    ...
    const fields = [
        ...
        { key: "architecture", label: "Architecture", placeholder: "Vertical Slice" },
    ];
    ...
    const handleSubmit = async (value: string) => {
        ...
        const newData = { ...formData, [currentField.key]: value || currentField.placeholder };
        setFormData(newData);
        setInputValue("");
        ...
        if (step < fields.length - 1) {
            ...
            try {
                const args = [
                    ...
                    "--user-role", newData.userRole,
                    "--working-style", newData.workingStyle,
                    "--architecture", newData.architecture,
                    "--force"
                ];
    ...
    return (
        <Box flexDirection="column" padding={1} borderStyle="single">
            <Text bold>Deep Bootstrap Interview ({step + 1}/{fields.length})</Text>
