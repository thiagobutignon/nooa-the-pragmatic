...
type LogLine = {
    type: "info" | "error" | "thinking" | "result" | "user" | "command" | "output";
    content: string;
};
...
export function ActScreen({ onBack }: ActScreenProps) {
        ...
        try {
            const result = await engineRef.current.execute(value, {
                ...
                onEvent: (event: ActEvent) => {
                    ...
                    if (event.type === "command") addLog("command", `Running: ${event.content}`);
    ...
    return (
        <Box flexDirection="column" padding={1} borderStyle="single" borderColor="cyan" height={30}>
            <Box flexDirection="row" justifyContent="space-between" marginBottom={1}>
                <Text bold color="cyan">NOOA Orchestrator (Direct)</Text>
            ...
            <Box flexDirection="column" flexGrow={1} borderStyle="single" borderColor="gray" paddingX={1}>
                ...
                {visibleLogs.map((log, i) => (
                    <Box key={startIndex + i} marginBottom={0} flexDirection="column">
                        {log.type === "user" && <Text color="green" bold>{log.content}</Text>}
                        {log.type === "thinking" && <Text color="yellow">  🤔 {log.content}</Text>}
                        {log.type === "command" && <Text color="blue">  ⚡ {log.content}</Text>}
                        {log.type === "output" && (
                            <Box marginLeft={2} borderStyle="single" borderColor="dim">
                                <Text dimColor>{log.content.slice(0, 200)}{log.content.length > 200 ? "..." : ""}</Text>
                            </Box>
                        )}
                        {log.type === "result" && (
            ...
            {/* Input Area */}
            <Box marginTop={1}>
                <Text color={isRunning ? "gray" : "green"}>{isRunning ? "Running..." : "Goal:"} </Text>
                {!isRunning && (
                    <TextInput
                        value={inputValue}
