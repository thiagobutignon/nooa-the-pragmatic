...
export async function searchAndMatchJobs(resumePath: string, query: string, providerKey: string = "arbeitnow") {
    ...
    if (providerKey === "arbeitnow") {
        ...
        const response = await fetch(`${provider.baseUrl}?search=${encodeURIComponent(query)}`);
        if (response.ok) {
            const json = (await response.json()) as any;
            const rawJobs = (json.data as any[]) || [];
            for (const rawJob of rawJobs) {
                const { score } = calculateMatchScore(resume, (rawJob.description || "") + " " + rawJob.title);
                const job: Job = {
                    provider: "arbeitnow",
        ...
        if (!owner || !repo) {
            throw new Error(`Invalid GitHub repository format: ${repoPath}. Expected owner/repo.`);
        }
        ...
        for (const job of githubJobs) {
            ...
            if (query && !job.title.toLowerCase().includes(query.toLowerCase()) && !job.description?.toLowerCase().includes(query.toLowerCase())) {
                continue;
            }

            const { score } = calculateMatchScore(resume, (job.description || "") + " " + job.title);
