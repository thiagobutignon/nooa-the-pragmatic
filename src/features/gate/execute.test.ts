...
describe("Gate Execute", () => {
    ...
    test("executes dogfood gate", async () => {
        const checkSpy = spyOn(gates.DogfoodGate.prototype, "check").mockResolvedValue({
            ok: true,
        });
    ...
    test("passes context correctly", async () => {
        let capturedCtx: any;
        const checkSpy = spyOn(gates.SpecGate.prototype, "check").mockImplementation(async (ctx) => {
            capturedCtx = ctx;
            return { ok: true };
        });
