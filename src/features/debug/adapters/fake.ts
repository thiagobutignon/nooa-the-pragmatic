import type { DebugBreakpointRef, DebugRuntime, DebugTarget } from "../session/types";
import type {
	DebugAdapter,
	DebugAttachInput,
...
class FakeDebugAdapter implements DebugAdapter {
	...
	async evaluate(input: DebugEvalInput): Promise<DebugEvalResult> {
		return {
			...
			value: `"fake:${input.expression}"`,
		};
	}

	private makeVars(entries: Array<[string, string]>): DebugValueSnapshot[] {
