...
export async function executeInit(options: InitOptions, bus?: any) {
    ...
    const context = {
        ...
        emoji: vibe === "snarky" ? "😼" : "🤖",
        avatar: "",
        tone_description: vibe === "snarky" ? "Subtle sarcasm, high technical rigor." : "Direct, efficient, and helpful."
    };
    ...
    if (!options.dryRun) {
        try {
            ...
            if (!options.force) {
                throw new Error(".nooa directory already exists. Use --force to re-initialize.");
            }
        } catch (e: any) {
            if (e.code === "ENOENT") {
    ...
    for (const file of files) {
        const tplContent = await Bun.file(file.tpl.startsWith("/") ? file.tpl : join(templatesDir, file.tpl)).text();
