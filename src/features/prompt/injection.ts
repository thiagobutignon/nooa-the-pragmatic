...
export class InjectionEngine {
    ...
    async getInjectedContext(): Promise<{ content: string; meta: InjectionMeta }> {
        const layers = [
            { id: "constitution", path: ".nooa/CONSTITUTION.md", budget: this.budgets.constitution, trusted: true },
            { id: "soul", path: ".nooa/SOUL.md", budget: this.budgets.soul, trusted: true },
            { id: "user", path: ".nooa/USER.md", budget: this.budgets.user, trusted: false },
            { id: "user_local", path: ".nooa/USER.local.md", budget: this.budgets.user, trusted: false },
            { id: "memory", path: ".nooa/MEMORY_SUMMARY.md", budget: this.budgets.memory, trusted: false }
        ];
        ...
        for (const layer of layers) {
            ...
            try {
                ...
                if (content.length > layer.budget) {
                    ...
                    meta.truncated = true;
                    if (!meta.truncatedLayers.includes(metaId)) meta.truncatedLayers.push(metaId);
                }
