...
export async function executeScaffold(options: ScaffoldOptions, bus?: any) {
    ...
    const engine = new ScaffoldEngine(join(root, "src/features/scaffold/templates"));

    // 1. Validation
    engine.validateName(options.name);

    const camelName = options.name.split("-").map((part, i) => i === 0 ? part : part.charAt(0).toUpperCase() + part.slice(1)).join("");

    const context: RenderContext & { camelName: string } = {
        name: options.name,
        camelName,
        Command: options.name.split("-").map(part => part.charAt(0).toUpperCase() + part.slice(1)).join(""),
        repo_root: root,
    ...
    if (options.type === "command") {
        ...
        const files = [
            ...
            { tpl: "command-execute-test.ts", path: join(featureDir, "execute.test.ts") }
        ];

        if (options.withDocs) {
            files.push({ tpl: "command-docs.md", path: join(root, "docs/commands", `${options.name}.md`) });
        }

        for (const file of files) {
            const content = await engine.renderTemplate(file.tpl, context);
            await engine.write(file.path, content, { force: options.force, dryRun: options.dryRun });
            results.push(file.path);
        }
    } else if (options.type === "prompt") {
        const promptPath = join(root, "src/features/prompt/templates", `${options.name}.md`);
        const content = await engine.renderTemplate("prompt-template.md", context);
        await engine.write(promptPath, content, { force: options.force, dryRun: options.dryRun });
