...
import ignoreCommand, { run } from "./cli";
...
describe("Ignore CLI", () => {
	...
	describe("run() handler", () => {
		...
		test("calls checkPathIgnored", async () => {
			const spy = spyOn(execute, "checkPathIgnored").mockResolvedValue({
				ignored: true,
				pattern: "*",
			});
	...
	describe("Command Execution (Output)", () => {
		...
		test("handles errors", async () => {
			const spy = spyOn(execute, "loadIgnore").mockRejectedValue(
				new Error("Boom"),
			);
			await ignoreCommand.execute({
				rawArgs: ["ignore", "list"],
		...
		test("handles unknown mode in switch", async () => {
			...
			const _runSpy = spyOn(execute, "addPattern"); // dummy
