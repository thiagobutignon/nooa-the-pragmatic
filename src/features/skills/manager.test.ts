...
describe("SkillManager", () => {
    ...
    test("createSkill scaffolds files", async () => {
        ...
        expect(mockMkdir).toHaveBeenCalledWith(join(skillsDir, "my-skill"), { recursive: true });
    ...
    test("deleteSkill removes directory", async () => {
        ...
        expect(mockRm).toHaveBeenCalledWith(join(skillsDir, "my-skill"), { recursive: true, force: true });
    });

    test("disableSkill creates .disabled file", async () => {
        await manager.disableSkill("my-skill");
        expect(mockWriteFile).toHaveBeenCalledWith(join(skillsDir, "my-skill", ".disabled"), "");
    });

    test("enableSkill removes .disabled file", async () => {
        await manager.enableSkill("my-skill");
        expect(mockRm).toHaveBeenCalledWith(join(skillsDir, "my-skill", ".disabled"), { force: true });
    });
    ...
    test("showSkill returns content", async () => {
        ...
        expect(mockReadFile).toHaveBeenCalledWith(join(skillsDir, "my-skill", "SKILL.md"), "utf-8");
