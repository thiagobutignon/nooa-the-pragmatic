import { describe, expect, spyOn, test, mock, beforeEach, afterEach } from "bun:test";
...
describe("Index Feature Integration", () => {
    ...
    test("semantic search quality regression (fixtures)", async () => {
        const embedSpy = spyOn(AiEngine.prototype, "embed").mockImplementation(async (opts) => {
            ...
            return { embeddings, model: "mock", provider: "mock", usage: { total_tokens: 0, prompt_tokens: 0, completion_tokens: 0 } } as any;
        });
        ...
        try {
            ...
            await fs.writeFile(`${tempDir}/auth/login.ts`, "export function login() {}");
            await fs.writeFile(`${tempDir}/auth/jwt.ts`, "export function signJwt() {}");
            await fs.writeFile(`${tempDir}/db/pool.ts`, "export function connectDb() {}");
