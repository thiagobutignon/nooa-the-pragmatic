import { describe, expect, spyOn, test, mock, beforeEach } from "bun:test";
import * as fs from "node:fs/promises";
import { store } from "../../core/db";
import { AiEngine } from "../ai/engine";
import * as execute from "./execute";

// Mock store before imports
const mockStore = {
	clear: mock(),
	stats: mock(),
	searchEmbeddings: mock(),
	storeEmbedding: mock(),
};

mock.module("../../core/db", () => ({
	store: mockStore,
}));

describe("Index Feature Execution (Unit)", () => {
	beforeEach(() => {
		mockStore.clear.mockReset();
		mockStore.stats.mockReset();
		mockStore.searchEmbeddings.mockReset();
		mockStore.storeEmbedding.mockReset();
		mockStore.clear.mockResolvedValue(undefined);
		mockStore.storeEmbedding.mockResolvedValue(undefined);
	});

	test("clearIndex calls store.clear", async () => {
		await execute.clearIndex();
		expect(mockStore.clear).toHaveBeenCalled();
	});

	test("getIndexStats calls store.stats", async () => {
		const expected = { documents: 10, chunks: 20 };
		mockStore.stats.mockResolvedValue(expected);

		const result = await execute.getIndexStats();

		expect(mockStore.stats).toHaveBeenCalled();
		expect(result).toEqual(expected);
	});

	test("indexRepo recursively lists and indexes files", async () => {
		const readdirSpy = spyOn(fs, "readdir").mockImplementation(
			async (path: any) => {
				if (path === ".")
					return [
						{ name: "a.ts", isDirectory: () => false },
						{ name: "dir", isDirectory: () => true },
					] as any;
				if (path === "dir")
					return [
						{ name: "b.md", isDirectory: () => false },
						{ name: "node_modules", isDirectory: () => true },
					] as any;
				return [];
			},
		);
		const readFileSpy = spyOn(fs, "readFile").mockResolvedValue(
			"content" as any,
		);
		const _aiSpy = spyOn(AiEngine.prototype, "embed").mockResolvedValue({
			embeddings: [[0.1, 0.2]],
		} as any);

		const result = await execute.indexRepo(".");

		expect(result.files).toBe(2); // a.ts and dir/b.md
		expect(readdirSpy).toHaveBeenCalledTimes(2); // . and dir
		expect(mockStore.storeEmbedding).toHaveBeenCalled();

		readdirSpy.mockRestore();
		readFileSpy.mockRestore();
		_aiSpy.mockRestore();
	});

	test("executeSearch embeds query and searches store", async () => {
		mockStore.searchEmbeddings.mockResolvedValue([
			{ path: "a.ts", chunk: "c1", score: 0.9 },
		]);
		const _aiSpy = spyOn(AiEngine.prototype, "embed").mockResolvedValue({
			embeddings: [[0.1, 0.2]],
		} as any);

		const results = await execute.executeSearch("query");

		expect(results).toHaveLength(1);
		expect(results[0]?.path).toBe("a.ts");
		expect(mockStore.searchEmbeddings).toHaveBeenCalled();
		_aiSpy.mockRestore();
	});

	test("rebuildIndex calls clear and indexRepo", async () => {
		const readdirSpy = spyOn(fs, "readdir").mockResolvedValue([] as any);

		await execute.rebuildIndex(".");

		expect(mockStore.clear).toHaveBeenCalled();
		readdirSpy.mockRestore();
	});

	test("chunking logic with large text", async () => {
		const readFileSpy = spyOn(fs, "readFile").mockResolvedValue(
			"p1\n\np2" as any,
		);

		const largeParagraph = "a".repeat(1100);
		readFileSpy.mockResolvedValue(
			`${largeParagraph}\n\n${largeParagraph}` as any,
		);
		const _aiSpy = spyOn(AiEngine.prototype, "embed").mockResolvedValue({
			embeddings: [[0.1, 0.2]],
		} as any);

		await execute.indexFile("test.ts", "test.ts");

		// Should split into multiple chunks
		expect(mockStore.storeEmbedding.mock.calls.length).toBeGreaterThan(1);

		readFileSpy.mockRestore();
		_aiSpy.mockRestore();
	});

	test("chunkText splits with overlap and respects line boundaries", () => {
		const input = Array.from({ length: 120 }, (_, i) => `line-${i}`).join("\n");
		const chunks = execute.chunkText(input, { maxChars: 200, overlapLines: 5 });
		expect(chunks.length).toBeGreaterThan(1);
		expect(chunks[0]).toContain("line-0");
		const lastLine = chunks[0].trim().split("\n").slice(-1)[0];
		expect(chunks[1]).toContain(lastLine ?? "");
	});

	test("chunkText keeps top-level boundaries when markers exist", () => {
		const input = [
			"export function alpha() {}",
			"",
			"export function beta() {}",
			"",
			"export class Gamma {}",
		].join("\n");
		const chunks = execute.chunkText(input, { maxChars: 50, overlapLines: 2 });
		expect(chunks.length).toBeGreaterThan(1);
		expect(chunks[0]).toContain("alpha");
		expect(chunks[1]).toContain("beta");
	});

	test("indexFile batches embedding calls", async () => {
		const content = Array.from({ length: 300 }, (_, i) => `line-${i}`).join(
			"\n",
		);
		const expectedChunks = execute.chunkText(content);
		const readFileSpy = spyOn(fs, "readFile").mockResolvedValue(content as any);

		const embedSpy = spyOn(AiEngine.prototype, "embed").mockResolvedValue({
			embeddings: Array.from({ length: expectedChunks.length }, () => [
				0.1, 0.2,
			]),
		} as any);

		const beforeCalls = embedSpy.mock.calls.length;
		await execute.indexFile("test.ts", "test.ts");

		expect(embedSpy.mock.calls.length - beforeCalls).toBe(1);
		expect(mockStore.storeEmbedding.mock.calls.length).toBeGreaterThan(1);

		readFileSpy.mockRestore();
		embedSpy.mockRestore();
	});

	test("executeSearch caches query embeddings with normalized keys", async () => {
		mockStore.searchEmbeddings.mockResolvedValue([
			{ path: "a.ts", chunk: "c1", score: 0.9 },
		]);
		const embedSpy = spyOn(AiEngine.prototype, "embed").mockResolvedValue({
			embeddings: [[0.1, 0.2]],
		} as any);

		const beforeCalls = embedSpy.mock.calls.length;
		await execute.executeSearch("Find   TODOs");
		await execute.executeSearch("find todos");

		expect(embedSpy.mock.calls.length - beforeCalls).toBe(1);
		embedSpy.mockRestore();
	});
});
