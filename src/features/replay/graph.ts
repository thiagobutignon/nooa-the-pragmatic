...
import type { ReplayEdge, ReplayGraph as GraphData, ReplayNode } from "./storage";
...
export class ReplayGraph {
    ...
    addNode(label: string, type: ReplayNode["type"] = "step", meta?: ReplayNode["meta"]): ReplayNode {
        if (!label) {
    ...
    addFix(targetId: string, label: string): ReplayNode {
        if (!this.nodes.has(targetId)) throw new Error(`Node ${targetId} not found`);
