...
export const actExamples = [
	{
		input: "nooa act 'Check status'",
		output: "Check the status of the project using the autonomous agent.",
	},
	{
		input: "nooa act 'Run CI and fix errors'",
		output: "Run CI and automatically fix any errors found.",
	},
];
...
const actBuilder = new CommandBuilder<ActRunInput, ActRunResult>()
	...
	.parseInput(async ({ positionals, values, bus, traceId }) => {
		const turnsRaw =
			typeof values.turns === "string" ? values.turns : undefined;
