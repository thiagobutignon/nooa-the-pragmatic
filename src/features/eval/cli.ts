...
export async function evalCli(args: string[]) {
    ...
    try {
        if (command === "run") {
            ...
            const result = await engine.runSuite(suite, { judge: values.judge as any });
            ...
            if (values.json) {
                ...
                for (const r of result.results) {
                    ...
                    console.log(`${status} Case: ${r.id} (Score: ${(r.score * 100).toFixed(0)}%)`);
            ...
            if (values["fail-on-regression"] && result.totalScore < 1.0) {
                logger.error("eval.regression", new Error(`Score ${result.totalScore} is below 1.0 threshold`));
                process.exitCode = 1;
            }
            ...
            if (result.totalScore < 1.0 && values["fail-on-regression"]) {
                console.error(`❌ Apply rejected: Score is ${(result.totalScore * 100).toFixed(0)}%.`);
                process.exitCode = 1;
                return;
            }
            ...
            const promptEngine = new PromptEngine(join(process.cwd(), "src/features/prompt/templates"));
            ...
            // For now, we mock the suggestion
            console.log("\n[DRAFT SUGGESTION]");
