...
export class EvalEngine {
    ...
    async loadSuite(name: string): Promise<EvalSuite> {
        const suitePath = join(process.cwd(), "src/features/eval/suites", `${name}.json`);
        const content = await readFile(suitePath, "utf-8");
        return JSON.parse(content);
    }

    async runSuite(suite: EvalSuite, options: { judge?: "deterministic" | "llm" } = {}): Promise<{ results: CaseResult[]; totalScore: number }> {
        ...
        const judgeTemplate = options.judge === "llm" ? await promptEngine.loadPrompt("eval-rubric") : null;
        ...
        for (const c of suite.cases) {
            ...
            const deterministicResult = this.scorer.score(response.content, c.assertions);
            ...
            if (judgeTemplate) {
                ...
                try {
                    ...
                    const judgeResult = JSON.parse(jsonMatch ? jsonMatch[0] : judgeResponse.content);
                    assertions.push({
                        passed: judgeResult.score >= 4,
                        message: `LLM Judge: ${judgeResult.critique} (Score: ${judgeResult.score}/5)`
                    });
                } catch (e) {
                    assertions.push({ passed: false, message: `LLM Judge failed to parse: ${(e as Error).message}` });
                }
            }
