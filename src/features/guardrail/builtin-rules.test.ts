...
describe("Built-in Profiles", () => {
    describe("Profile Loading", () => {
        const profilesDir = join(
            process.cwd(),
            ".nooa/guardrails/profiles",
        );

        it("should load zero-preguica profile", async () => {
            const profile = await loadProfile(join(profilesDir, "zero-preguica.yaml"));
    ...
    describe("Security Detection", () => {
        ...
        beforeAll(async () => {
            ...
            await writeFile(join(tempDir, "secure.ts"), `const x = process.env.API_KEY;`);
