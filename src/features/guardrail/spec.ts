...
import { loadBuiltinProfile, type BuiltinProfileName } from "./builtin";
...
export async function parseGuardrailSpec(
    ...
    const path = specPath ?? join(process.cwd(), ".nooa", "guardrails", "GUARDRAIL.md");
...
function extractCustomRules(content: string): RefactorRule[] {
    const rulesSection = content.match(/## Custom Rules[\s\S]*?```yaml([\s\S]*?)```/);
