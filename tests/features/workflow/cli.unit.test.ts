...
import type { SdkResult } from "../../../src/core/types";
import {
...
import type {
	WorkflowRunInput,
	WorkflowRunResult,
} from "../../../src/features/workflow/execute";
...
describe("workflow CLI unit tests", () => {
	...
	describe("handleWorkflowSuccess", () => {
		...
		it("should print failure message and set exit code if not ok", () => {
			...
			handleWorkflowSuccess(
				{ ok: false, failedStepId: "step1", reason: "bad" },
				{},
			);

			expect(errorSpy).toHaveBeenCalledWith(
				expect.stringContaining("Workflow failed at step: step1"),
			);
	...
	describe("run", () => {
		it("should call injected runner with parsed args", async () => {
			...
			const result = await run(
				{ action: "run", gates: "spec,test" },
				mockRunner,
			);

			expect(result.ok).toBe(true);
			expect(mockRunner).toHaveBeenCalled();
			expect(mockRunner.mock.calls[0][0]).toMatchObject({
				gates: ["spec", "test"],
			});
		});
