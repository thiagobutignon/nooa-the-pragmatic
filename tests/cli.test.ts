...
describe('CLI (index.ts)', () => {
    ...
    const dummyRef = join(process.cwd(), 'tests/dummy.pdf');
    ...
    it('should handle missing files gracefully', async () => {
        const { stderr, exitCode } = await execa('bun', [binPath, 'non-existent.pdf'], { reject: false });
    ...
    it('should fail if --to-pdf used without input markdown', async () => {
        const { stderr, exitCode } = await execa('bun', [binPath, '--to-pdf'], { reject: false });
        expect(exitCode).not.toBe(0);
