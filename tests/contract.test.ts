...
describe("NOOA Contract Tests", () => {
    ...
    describe("Init Contract", () => {
        it("should create .nooa directory and core files", async () => {
            ...
            const soulContent = await readFile(join(TEST_ROOT, ".nooa/SOUL.md"), "utf-8");
        ...
        it("should fail if .nooa already exists without --force", async () => {
            ...
            expect(executeInit({ root: TEST_ROOT } as any)).rejects.toThrow("already exists");
        });

        it("should succeed if .nooa already exists with --force", async () => {
            await mkdir(join(TEST_ROOT, ".nooa"), { recursive: true });
            const { results } = await executeInit({ root: TEST_ROOT, force: true } as any);
            expect(results.length).toBeGreaterThan(0);
        });
    });
    ...
    describe("Memory Contract", () => {
        it("should generate valid YAML entries", async () => {
            ...
            const content = await readFile(join(TEST_ROOT, `memory/${date}.md`), "utf-8");
        ...
        it("should promote entries to durable memory", async () => {
            ...
            const durableContent = await readFile(join(TEST_ROOT, ".nooa/MEMORY.md"), "utf-8");
    ...
    describe("Reflection Contract", () => {
        it("should only reflect on material events", async () => {
            ...
            // Non-material event
            await reflector.reflect({ event: "search.query", level: "info", success: true } as any);
        ...
        it("should reflect on material events", async () => {
            ...
            await reflector.reflect({ event: "commit.success", level: "info", success: true, trace_id: "t-1" } as any);
        ...
        it("should cap reflections at 3 per session", async () => {
            ...
            for (let i = 0; i < 5; i++) {
                await reflector.reflect({ event: "commit.success", level: "info", success: true } as any);
            }
