---
summary: "How Nooa memory works (workspace files + automatic memory flush)"
read_when:
...
Nooa memory is **plain Markdown in the agent workspace**. The files are the
...
When a session is **close to auto-compaction**, Nooa triggers a **silent,
...
Nooa can build a small vector index over `MEMORY.md` and `memory/*.md` (plus
...
- Uses remote embeddings by default. If `memorySearch.provider` is not set, Nooa auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. Nooa
...
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, Nooa automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, Nooa combines:
- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, Nooa falls back to vector-only search.
...
Nooa can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
When the sqlite-vec extension is available, Nooa stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, Nooa logs the
